...
$APPLICATION->SetPageProperty("TITLE", "Купить контейнеры для майнинга — Gis Mining");
$APPLICATION->SetTitle("Контейнеры");
// Хлебные крошки теперь формируются автоматически в header
$APPLICATION->SetPageProperty("description", "Контейнеры для майнинга от компании Gis Mining. Разнообразие моделей для эффективного размещения майнингового оборудования.");
$APPLICATION->SetPageProperty("robots", "index, follow");

// --- OPEN GRAPH МЕТА-ТЕГИ ---

$APPLICATION->SetPageProperty("OG:TITLE", "Купить контейнеры для майнинга — Gis Mining");
$APPLICATION->SetPageProperty("OG:DESCRIPTION", "Контейнеры для майнинга от компании Gis Mining. Разнообразие моделей для эффективного размещения майнингового оборудования.");
...
$APPLICATION->SetPageProperty("TWITTER:TITLE", "Купить контейнеры для майнинга — Gis Mining");
$APPLICATION->SetPageProperty("TWITTER:DESCRIPTION", "Контейнеры для майнинга от компании Gis Mining. Разнообразие моделей для эффективного размещения майнингового оборудования.");
...
<div class="catalog-page catalog-new container" id="app-root" data-iblock-id="<?= $IBLOCK_ID ?>">
    <h1 class="catalog-page__title section-title highlighted-color">Контейнеры для майнинга</h1>
        ...
        <section class="catalog-page__content section-padding">
            ...
            $APPLICATION->IncludeComponent(
                ...
                array(
                    ...
                    "SECTION_URL" => "/catalog/conteynery/",
    ...
    <section class="catalog-about section-padding">
        <div class="about__content">
            <h2 class="about__title"><?= $iblockData['NAME'] ?: 'Контейнеры' ?></h2>
...
<!-- PopUp -->
<div class="page-business__popup-form popup-form-wrapper js-cart-modal" id="mainPopupFormWrapper" style="display: none;">

    <!-- ВАЖНО: У формы теперь есть класс js-ajax-form, чтобы ее "подхватил" form-actions.js -->
    <form class="popup-form js-ajax-form" id="conteyneryCartForm" data-metric-goal="send-container-lead">
        ...
        <p class="popup-form__text">Получить КП на контейнеры</p>
        ...
        <input type="text" name="client_name" id="cart_client_name" placeholder="Имя" class="contact-form__input form-input" required aria-label="Имя">

        <label for="cart_client_phone">Телефон*:</label>
        <input type="tel" name="client_phone" id="cart_client_phone" placeholder="Телефон*" class="contact-form__input form-input js-phone-mask" required aria-label="Номер телефона">

        <label for="cart_client_email">Email:</label>
        <input type="email" name="client_email" id="conteynery_cart_client_email" placeholder="your@email.com (необязательно)" class="contact-form__input form-input" aria-label="Электронная почта">

        <input type="hidden" name="source_id" value="44">
        ...
        <input type="hidden" name="form_name" value="Заказ из каталога контейнеров">
