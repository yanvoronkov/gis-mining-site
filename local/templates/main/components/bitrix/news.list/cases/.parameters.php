<?php
if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();

$arComponentParameters = array(
    "GROUPS" => array(
        "SETTINGS" => array(
            "NAME" => "Настройки отображения",
            "SORT" => 100,
        ),
        "FILTER" => array(
            "NAME" => "Настройки фильтрации",
            "SORT" => 200,
        ),
        "SEO" => array(
            "NAME" => "SEO настройки",
            "SORT" => 300,
        ),
    ),
    "PARAMETERS" => array(
        "IBLOCK_TYPE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Тип инфоблока",
            "TYPE" => "LIST",
            "VALUES" => $arIBlockType,
            "REFRESH" => "Y",
        ),
        "IBLOCK_ID" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "ID инфоблока",
            "TYPE" => "LIST",
            "VALUES" => $arIBlock,
            "REFRESH" => "Y",
            "ADDITIONAL_VALUES" => "Y",
        ),
        "NEWS_COUNT" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Количество элементов на странице",
            "TYPE" => "STRING",
            "DEFAULT" => "20",
        ),
        "SORT_BY1" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Поле для первой сортировки",
            "TYPE" => "LIST",
            "DEFAULT" => "ACTIVE_FROM",
            "VALUES" => array(
                "ACTIVE_FROM" => "Дата начала активности",
                "ACTIVE_TO" => "Дата окончания активности",
                "SORT" => "Индекс сортировки",
                "NAME" => "Название",
                "ID" => "ID",
                "CREATED" => "Дата создания",
                "TIMESTAMP_X" => "Дата изменения",
            ),
        ),
        "SORT_ORDER1" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Направление для первой сортировки",
            "TYPE" => "LIST",
            "DEFAULT" => "DESC",
            "VALUES" => array(
                "ASC" => "По возрастанию",
                "DESC" => "По убыванию",
            ),
        ),
        "SORT_BY2" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Поле для второй сортировки",
            "TYPE" => "LIST",
            "DEFAULT" => "SORT",
            "VALUES" => array(
                "ACTIVE_FROM" => "Дата начала активности",
                "ACTIVE_TO" => "Дата окончания активности",
                "SORT" => "Индекс сортировки",
                "NAME" => "Название",
                "ID" => "ID",
                "CREATED" => "Дата создания",
                "TIMESTAMP_X" => "Дата изменения",
            ),
        ),
        "SORT_ORDER2" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Направление для второй сортировки",
            "TYPE" => "LIST",
            "DEFAULT" => "ASC",
            "VALUES" => array(
                "ASC" => "По возрастанию",
                "DESC" => "По убыванию",
            ),
        ),
        "FILTER_NAME" => array(
            "PARENT" => "FILTER",
            "NAME" => "Имя фильтра",
            "TYPE" => "STRING",
            "DEFAULT" => "",
        ),
        "FIELD_CODE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Поля для выборки",
            "TYPE" => "LIST",
            "MULTIPLE" => "Y",
            "VALUES" => array(
                "ID" => "ID",
                "NAME" => "Название",
                "PREVIEW_TEXT" => "Текст анонса",
                "PREVIEW_PICTURE" => "Картинка анонса",
                "DETAIL_TEXT" => "Детальный текст",
                "DETAIL_PICTURE" => "Детальная картинка",
                "ACTIVE_FROM" => "Дата начала активности",
                "ACTIVE_TO" => "Дата окончания активности",
                "SORT" => "Индекс сортировки",
                "CREATED" => "Дата создания",
                "TIMESTAMP_X" => "Дата изменения",
            ),
            "DEFAULT" => array("ID", "NAME", "PREVIEW_TEXT", "PREVIEW_PICTURE", "ACTIVE_FROM"),
        ),
        "PROPERTY_CODE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Свойства для выборки",
            "TYPE" => "LIST",
            "MULTIPLE" => "Y",
            "VALUES" => $arProperty_LNS,
            "DEFAULT" => array("TAGS"),
        ),
        "CHECK_DATES" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Показывать только активные на данный момент элементы",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "Y",
        ),
        "DETAIL_URL" => array(
            "PARENT" => "SEO",
            "NAME" => "URL страницы детального просмотра",
            "TYPE" => "STRING",
            "DEFAULT" => "",
        ),
        "AJAX_MODE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Включить режим AJAX",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
        ),
        "AJAX_OPTION_JUMP" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Включить прокрутку к началу компонента",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
            "DEPENDENT_PARAMETERS" => array(
                "AJAX_MODE" => "Y",
            ),
        ),
        "AJAX_OPTION_STYLE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Включить подгрузку стилей",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "Y",
            "DEPENDENT_PARAMETERS" => array(
                "AJAX_MODE" => "Y",
            ),
        ),
        "AJAX_OPTION_HISTORY" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Включить эмуляцию навигации браузера",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
            "DEPENDENT_PARAMETERS" => array(
                "AJAX_MODE" => "Y",
            ),
        ),
        "CACHE_TYPE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Тип кеширования",
            "TYPE" => "LIST",
            "VALUES" => array(
                "A" => "Авто + Управляемое",
                "Y" => "Кешировать",
                "N" => "Не кешировать",
            ),
            "DEFAULT" => "A",
        ),
        "CACHE_TIME" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Время кеширования (сек.)",
            "TYPE" => "STRING",
            "DEFAULT" => "36000000",
        ),
        "CACHE_FILTER" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Кешировать при установленном фильтре",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
        ),
        "CACHE_GROUPS" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Учитывать права доступа",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "Y",
        ),
        "DISPLAY_TOP_PAGER" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Выводить над списком",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
        ),
        "DISPLAY_BOTTOM_PAGER" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Выводить под списком",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "Y",
        ),
        "PAGER_TEMPLATE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Название шаблона",
            "TYPE" => "STRING",
            "DEFAULT" => "",
        ),
        "PAGER_SHOW_ALWAYS" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Выводить всегда",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
        ),
        "PAGER_DESC_NUMBERING" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Использовать обратную навигацию",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
        ),
        "PAGER_DESC_NUMBERING_CACHE_TIME" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Время кеширования страниц для обратной навигации",
            "TYPE" => "STRING",
            "DEFAULT" => "36000",
        ),
        "PAGER_SHOW_ALL" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Показывать ссылку 'Все'",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
        ),
        "PAGER_BASE_LINK_ENABLE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Включить обработку базовых ссылок",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "N",
        ),
        "PAGER_BASE_LINK" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Базовые ссылки для постраничной навигации",
            "TYPE" => "STRING",
            "DEFAULT" => "",
        ),
        "PAGER_PARAMS_NAME" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Имя параметра с номером страницы",
            "TYPE" => "STRING",
            "DEFAULT" => "PAGEN_",
        ),
        "SHOW_FEATURED_ARTICLE" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Показывать главную статью",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "Y",
        ),
        "SHOW_FILTER" => array(
            "PARENT" => "SETTINGS",
            "NAME" => "Показывать фильтр по разделам",
            "TYPE" => "CHECKBOX",
            "DEFAULT" => "Y",
        ),
    ),
);
