        ...
        <div class="product-grid asic-miners__product-grid">
            <?php foreach ($arResult["SEARCH"] as $arItem): ?>
                ...
                <?php if ($arItem["IS_PRODUCT"] && !empty($arItem["PRODUCT_DATA"])): ?>
                    ...
                    <div class="product-card asic-product-card" data-product-id="<?= $product['ID'] ?>"
                        data-name="<?= htmlspecialchars($product['NAME']) ?>" data-price="<?= htmlspecialchars($priceFormatted) ?>"
                        data-price-raw="<?= $priceRaw ?>" data-photo="<?= htmlspecialchars($imageSrc) ?>">
